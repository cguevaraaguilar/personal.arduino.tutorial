...
String str_step(int i) {
  ...
  if (steps[i].check == 1) s = s + "+"; else if (steps[i].check == 2) s = s + "#";
  return s;
}
//****************************
void show_steps() {
...
int alphaBeta(int start, int nstep, int alpha, int beta, int depthleft) {
  ...
  for (int i = start; i < j; i++) { //
    if (nstep == cur_step) {
      ...
      if (steps[i].weight < -9000) {
        Serial.println(F(" checkmate"));
        continue;
      }
